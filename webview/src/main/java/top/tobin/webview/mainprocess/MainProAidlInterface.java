package top.tobin.webview.mainprocess;

import android.content.Context;
import android.os.RemoteException;
import android.util.Log;

import top.tobin.webview.IWebAidlCallback;
import top.tobin.webview.IWebAidlInterface;
import top.tobin.webview.command.mainprocess.MainProcessCommandsManager;
...
public class MainProAidlInterface extends IWebAidlInterface.Stub {
    ...
    public void handleWebAction(int level, String actionName, String jsonParams, IWebAidlCallback callback) throws RemoteException {
        ...
        Log.d("MainProAidlInterface", String.format("MainProAidlInterface: 进程ID（%d）， WebView请求（%s）, 参数 （%s）", pid, actionName, jsonParams));
        try {
